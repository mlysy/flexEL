---
title: "<code>flexEL</code>: A Flexible Package for Empirical Likelihood Regression Models"
author: ""
date: "`r Sys.Date()`"
output:
...
rmarkdown::render("R_model_object.Rmd")
```
...
The **log EL function** is defined by
\[
...
\[
  \E[g(x,y;\tth)] = 0.
\]
This function provides all information about the parameter $\tth$ of interest and is often called the estimating fucntion for $\tth$. 

The above inner optimization problem to find $\hat{\w}$ given $\tth$ can be solved efficiently through a Newton-Raphoson algorithm, more detailed explanation can be found here (TODO: link to another document?).
...
In later section explaining the usage of this package, we will see that specifying the $G$ matrix according to the problem one would like to solve is the only requirement to use this package.

### EL for Right-censored Responses

When the response is time-to-event data such as survial time in many medical studies, one situation that often occurs is right-censoring, one type of length-bias problems. That is, instead of observing the true survival time for each individual, what being observed is either the true survival time, or the time this person dropped out of the study. To be precise, instead of observing $y_i$, we observe $u_i = \min(y_i,c_i)$ and $\delta_i=\mathfrak 1(y_i\leq c_i)$, where $c_i$ is a censoring variable that is assumed to be independent of $y_i$ given covariate $x_i$. 

The log EL function under right-censoring (CEL) is defined as
\[
...
Due to right-censoring, this inner optimization problem turns out to be difficult to solve directly. Instead, we use an EM algorithm to solve it. However, the EL function in this case is not continous with respect to $\tth$, to deal with this problem, we introduce a continuity-correction in the following subsection.
...
Upon observing data, the purpose is to find the estimates of the parameters of your semi-parametric regression model. To achieve this, two steps needed to be done:

1. Construct the log EL function according to your model
2. Obtain an estimate based on the log EL function

In the first step, the only key element required from the user a function evaluating the $G$ matrix which is explained in the previous section.
...
source("../tests/dontrun/gen_eps.R")
n <- 500 # number of observations
b <- c(0.5,1) # beta_0 = 0.5, beta_1 = 1
eps <- gen_eps(n, dist="nct", df=20, ncp=1) # a re-centered right-skewed non-central t distribution
X <- cbind(1,rnorm(n)) # n x 2 covariate matrix (intercept included)
y <- X %*% b + eps # length n response vector
```
We explain how to use this package to conduct inference for your regression model through some implementations either in R or in C++ in the following two sections.

### Usage in R

As the first step, for any particular regression model, one should design an estimating function of the parameters, and implement a function (`evalG`) to return the corresponding $G$ matrix of dimension $n\times m$, where $n$ is the number of observations, and $m$ is the dimension of the range of the estimating equation. This function can be directly implemented in R.
...
This can be implemented in R (`mr.evalG_R`) which returns the $G$ matrix: 
```{r, echo=TRUE, eval=TRUE}
...
mr.neglogEL_R <- function(y, X, beta) {
  G <- mr.evalG_R(y, X, beta) # G matrix based on your regression problem
  omegas <- omega.hat(G) # the empirical distribution provided by flexEL
  if (!anyNA(omegas)) return(-logEL(omegas)) # returns the negative log EL, if optimal omegas is found
  else return(Inf) # otherwise, returns Inf
}
```
...
```{r, echo=TRUE, eval=TRUE}
library(flexEL)
beta.init <- coef(lm(y ~ X-1)) # obtain initial value
nlmout <- nlm(f=mr.neglogEL_R, p=beta.init, y=y, X=X)
nlmout$estimate
```


You may also provide the gradient of the estimating equations to the optimization algorithm, which can speed up the optimization. (TODO: gradient requires lamba, needs more explanations)
```{r, echo=TRUE, eval=TRUE}
source("../tests/dontrun/mode-functions.R")
mr.neglogEL_R <- function(y, X, beta) {
...
TODO: Talk about how user can supply a C++ header file to construct $G$ matrix efficiently and exported as R function to solve their EL problem. Use the above mean regression example. 

## Models Included in the Package

TODO: Talk about the mean and quantile regression included in this pacakge. Explain smoothing 

## Other Optimization / Modification Methods Included in the Package

TODO: If Aitken acceleration for EM and adjusted G matrix for support modification are included, talk about them here.

## TODO

TODO: Describe ongoing and to-be-done works. 
