...
mr_neglogEL_R <- function(y, X, beta) {
  ...
  omegas <- flexEL::omega_hat(G) # the empirical distribution provided by flexEL
  if (!anyNA(omegas)) return(-flexEL::logEL(omegas)) # returns the negative log EL, if optimal omegas is found
...
```{r, echo=TRUE, eval=TRUE}
logEL_grad_R <- function(omegas, lambda, gradlist) {
...
One can also implement a regression model in C++ following the header file `mean_reg_model.h`. The key element of this is once again the `EvalG` method which defines the regression model. 
...
// calculate EL for a particular beta
