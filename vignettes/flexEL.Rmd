...
In fact, specifying the $G$ matrix according to your regression model is the only requirement to use this package.

### EL for Right-censored Responses

When the response is time-to-event data such as survial time in many medical studies, one situation that often occurs is right-censoring, one type of length-bias problem. That is, instead of observing the true survival time for each individual, what being observed is either the true survival time or the time this person dropped out of the study. Specifially, instead of observing $y_i$, we observe $u_i = \min(y_i,c_i)$ and $\delta_i=\mathfrak 1(y_i\leq c_i)$, where $c_i$ is a censoring variable that is assumed to be independent of $y_i$ given covariate $x_i$. 
...
mr_neglogEL_R <- function(y, X, beta) {
  ...
  omegas <- omega_hat(G) # the empirical distribution provided by flexEL
...
nlmout <- nlm(f=mr_neglogEL_R, p=beta.init, y=y, X=X)
nlmout$estimate
```
...
### Location-Scale Mean Regression Model
