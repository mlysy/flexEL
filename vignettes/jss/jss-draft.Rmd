...
author: 
  ...
  - name: Yunfeng Yang
    affiliation: University of Waterloo
  - name: Silvia Meng
    affiliation: 'University of Waterloo \AND'
...
knitr::opts_chunk$set(comment="",
                      prompt = TRUE,
                      eval = FALSE, # set to TRUE to get actual output
...
When $q_i = 1, \forall i=1,\cdots,n$, we have the regular empirical likelihood function. The problem in \eqref{eq:noncensopt} is a constrained convex optimization problem, and its optimal solution can be found via Lagrangian function, similar to the steps in @owen1990. Specifically, the Lagrangian function can be set up as 
...
With the Lagrangian function in \eqref{eq:lagrange}, we can also derive the gradient of $\L$ with respect to $\tth$, which allows one to employ a gradient-based optimization algorithm to find the optimal solution of the estimator. Specifically, the gradient of the the weighted empirical log likelihood with respect to $\tth$ can be derived as
...
Let us denote the residuals given a specific $\tth$ as $e_i = e_i(\tth) = \{u_i - \mu(\xx_i;\tth)\}/\eta(\xx_i;\tth)$ and the complete residuals as $\e_i = \{y_i - \mu(\xx_i;\tth)\}/\eta(\xx_i;\tth)$. The empirical likelihood with censored observations once again is defined by profiling over the unknown joint distribution function $F(\xx,\e) = G(\xx) \cdot H(\e)$, where $G$ and $H$ are the CDFs of $\xx$ and $\e$:
...
We can see that the discontinuity of $\ell_{\CEL}(\tth)$ in \eqref{eq:logel_orig} is due to an indicator function. To correct this discontinuity, we replace the indicator function by a continuous approximation.

Let $S$ be a transformed sigmoid function, i.e.,
\begin{equation}\label{eq:sigmoid}
  S(x; s) = \frac{1}{1+\exp(s\cdot x)},
\end{equation}
where $s > 0$ is a smoothing parameter. A plot of the function is given in Figure \eqref{fig:smoothfun}. This function is radially symmetric around the point $(0,0.5)$. The smoothing parameter $s$ is default to 10 in \pkg{flexEL}.
...
\end{equation}
Notice that as long as $e_i(\tth)$ is a continuous function of $\tth$ for all $i=1,\cdots,n$, the function \eqref{eq:indsmooth} is indeed a continuous function of $\tth$.
...
<!-- The E-step of the EM algorithm takes the expectation of (\eqref{eq:complike}) with respect to $\yy$ (vector of all latent variables), conditioned on the observed values, the censoring indicator and the current state of the parameters. Since $\delta_i = 1$ (meaning $u_i=y_i$), we have -->
...
<!-- The conditional distribution in (\eqref{eq:condprob_orig}) is a categorical distribution conditioned on that the probability mass is allocated in a subset of $\{e_1,\cdots,e_n\}$ such that $\mathfrak 1(e_j\geq e_i) = 1$ for $j=1,\cdots,n$, which is still a multinomial distribution, -->
...
<!-- With smoothing function (\eqref{eq:indsmooth}), the probability (\eqref{eq:condprob_orig}) can be converted to   -->
...
which has the form of the weighted empirical likelihood inner optimization problem, and $\oom = (\w_1,\cdots,\w_n)$ can be updated as given by equations \eqref{eq:omegahat} to \eqref{eq:optim}.
...
The BayesEL method combines the prior information on parameters and the data-driven likelihood to draw inference on the estimand that is defined by estimating equations. With the prior density $\pi(\tth)$ over the support $\Theta$ and the likelihood $L(\xx\mid\tth)$, the posterior is defined as:

\begin{equation}
\pi(\tth\mid\xx) = \frac{L(\xx\mid\tth)\pi(\tth)}{\int L(\xx\mid\tth)\pi(\tth) \ud \tth}
...
- [x] Define the model.  This is basically given by (24).  Please see \pkg{flexEL} vignette for details on the model.

- [x] Write down the moment conditions.  We need to know what $\gg(\xx, y; \bbe)$ is in order to implement the EL model.  In this case it is

	$$
	\gg(\xx, y; \bbe) = (y - \xx'\bbe) \xx.
	$$

- [x] Also need to write down the gradient, i.e., $\frac{\ud}{\ud \bbe} \gg(\xx, y; \bbe)$.

- [ ] Clean up the code.  This we can do after the math is written down correctly.


Let us consider a general multivariate linear regression as an example. Suppose we have $n$ independent and identically distributed observations $(\xx_i,y_i)$, $i = 1,...,n$, which follow the model 
\begin{equation}
  y_i = \xx_i'\bbe + \e_i, \quad i=1,\cdots,n.,
\end{equation}
where $\epsilon_i$ follows an unknown distribution with $\E(\epsilon_i) = 0$.

To implement the empirical likelihood, the moment condition in this example is 
\begin{equation}
\gg(\xx, y; \bbe) = (y - \xx'\bbe) \xx.
\end{equation}

And the gradient of $\gg(\xx, y; \bbe)$ with respect to $\bbe$ is 
\begin{equation}
\frac{\ud}{\ud \bbe} \gg(\xx, y; \bbe) = -\xx\xx'.
\end{equation}


We first show the simple functional implementation of \pkg{flexEL}. The function `meanreg_log()` provides the logEL value based on the `G` matrix and `flexEL::logEL()`. The fastest way to implement \pkg{flexEL} is to use `nlm()` to minimize the negative logEL value and get $\bbe$ estimate. 


<!-- By creating the function `mr_evalG_R()` returning `G` matrix and `mr_neglogel()` returning negative logEL value, one can use `nlm()` to minimize the negative logEL value and get $\bbe$ estimate.  -->


```{r functional_implementation}
require(flexEL)
...
Further, one can use a more flexible and memory efficient coding, which is the object-oriented programming (OOP). We also include the gradient of logEL with respect to $\bbe$, $\frac{\ud}{\ud \bbe} \log \EL(\bbe)$, in the object-oriented programming.
...
Next, we adopt the two-parameter model as an example for illustration. In the data simulation, we set $\bbe = \left[\beta_0, \beta_1\right]' = \left[1,2\right]'$, $\xx = \left[1, x\right]'$, where $x \iid \N(0,1)$, and the sample size $n= 200$. 
...
Then, we show that the functional implementation and the object oriented programming give the same logEL value and the gradient with the simulated data. 

```{r result_check }

#' check the logEL is the same 
# the logEL value in the functional implementation
...
#' functional result 
#' the functional way returning the gradient
...
#' the function returning negative logEL value and dldb
...
attr(mr_neglogEL_R(y,X,c(0.75, 1.25)),"gradient")

#' object-oriented result of the gradient 
-mrel$dldb(c(0.75, 1.25))
```

Finally, we perform the optimization to obtain the parameter estimate of $\bbe$, $\hat{\bbe}$.


```{r optimization_implementation}
#' Calculate the negative logEL in the mean regression model
neg_meanreg_logel <- function(beta) {
  -meanreg_logel(y,X,beta)
}
nlmresult <- nlm(neg_meanreg_logel, c(0.75, 1.25), hessian = TRUE)
nlmresult
# nlm(meanreg_logel, c(0.75, 1.25), y, X)
```

And we also demonstrate the standard error of $\hat{\bbe}$, $\text{se}(\hat{\bbe})$.

```{r sd_bbe}
# save the beta_hat estimate
beta_hat <- nlmresult$estimate
# calculate the fisher information matrix
FI <- nlmresult$hessian
# calculat the standard error of the beta_hat
beta_se <- sqrt(diag(chol2inv(chol(FI))))
beta_se
```


<!-- Further, we can adopt a more object-oriented implementation. Here, we show that the functional implementation and the object-oriented implementation provide the same logEL value and gradient. -->
...
<!-- Further, in this section, we illustrate how one can interact with the C++ API or \proglang{R} API with the built-in location mean regression model and achieve parameter estimation in \proglang{R}.  -->

<!-- The following script creates a function in C++ that can be exported to \proglang{R} using Rcpp: -->
<!-- ```{r mrcpp, echo = FALSE, results = "asis"} -->
...
<!--  - [ ] The following coding reports error  -->
<!-- ```{r mrcppsource, warning=FALSE} -->
<!-- # report error -->
...
<!-- # uncommment the following sentence -->
...
As before, we use the first order optimality condition of \eqref{eq:nueq} to obtain the estimating equation for $\nu_{\tau}$. Therefore, we obtain all the moment conditions for quantile regression as follows
...
\begin{split}
  ...
  \E\Bigl[\rho^{(1)}_{\tau}\bigl(\frac{y-\xx'\bbe}{\s\cdot\exp(\zz'\gga)}-\nu_{\tau}\bigr)\Bigr] &= 0,
\end{split}
\end{equation}

where $\rho^{(1)}_{\tau}(u) = \frac{\ud}{\ud u} \rho_{\tau}(u)$.
...
\end{equation}
where $S$ is defined as in \eqref{eq:sigmoid}. Also, the gradient is derived as 

\begin{equation}
\frac{\ud}{\ud u} \rho_{S,\tau}(u; s) = (\tau - \frac{1}{1 + \exp(s\cdot u)}) + u\cdot \frac{s\cdot \exp(s\cdot u)}{\left[1 + \exp(s\cdot u)\right]^2},
\end{equation}
...
- [x] Write down the derivative of the smoothed check function $\frac{\ud}{\ud u} \rho_{S,\tau}(u; s)$, and complete the coding in `check_smooth_dx()`.
...
(Check the gradient by using `numDeriv::grad()`. After many experiments, I found that the `numDeriv::grad()` does not allow the function to have a argument named `s`. That is why I change `s` to `sm` in the gradient check. 

```{r gradient_check}
# generate the random `x`
x <- rnorm(10)
# our `check_smooth_dx` resultwith `tau = .5`, `s = 1`
check_smooth_dx(x = x, tau = .5, s = 1)

# next we use `numDeriv::grad()`
check_smooth <- function(x, tau, sm) {
  x * (tau - 1/(1 + exp(sm * x)))
}
numDeriv::grad(check_smooth,  x, tau = .5, sm = 1)
...
Next, we follow an example introduced in @huang18. We consider the location-scale right-censored quantile regression model:

\begin{equation} 
  y_i = \xx_i'\bbe + \s\cdot\exp(\zz_i'\gga)\cdot\epsilon_i, \quad i=1,\cdots,n,
\end{equation}

In the data simulation, we set $\xx_i = \left[1,x_i\right]'$, $\bbe = \left[\beta_0, \beta_1\right]' = \left[0.5,1\right]'$, $\sigma^2 = 1$. Also, we let $x_i,z_i \iid \N(0,1)$, the censoring variable $c_i \iid \N(1.35,1)$, and $\epsilon_i =\min(\e_i,c_i)$, where $\e_i \iid \N(0,1)$. We fit $75$-th quantile regression in this example, that is $\tau = 0.75$.

We compare the estimators obtained by using \pkg{flexEL} and the heteroscedastic accelerated failure time model by @wang15 with Kaplan-Meier (HLM+KM).


```{r quantreg_cel_datagen}
set.seed(20220811)
...
tau <- 0.75 
# Let the initial value be the true value 
beta <- c(.5, 1) 
gamma <- -.5 
sig2 <- 1 
sigma <- sqrt(sig2)
...
delta <- eps <= c # censoring indicators
y_allcen <- c(X %*% beta0 + sqrt(sig20)*exp(Z %*% gamma0)*c)
y_obs <- y
y_obs[!delta] <- y_allcen[!delta]
```

```{r quantreg_fit}
# set s = 1 in the smoothing method
s <- 1
# generate the quantreg class
qrel <- QuantReg$new(y_obs, delta, X, Z, s, tau)
# turn supp_adj on
qrel$supp_adj <- TRUE
# use the smooth method
qrel$smooth <- TRUE
# set s = 1 in the smoothing method
qrel$smooth_s <- 1


# negative logel of quantile regression 
...
# optim result
optimout <- optim( theta_init,neg_quantreg_logel)
optimout$par
optimout$converge

# Note that we let the true value be the initial value
rbind(theta_init, optimout$par)
```
...
require(hlm)
# fit the hlm model
M <- hlm(cbind(y_obs,delta)~ X + 0|Z+0)
# Note that M$coefficients$gamma/2 is the estimator of gamma
M
# calculate the eps
uXb <- y_obs - as.numeric(X %*% M$coefficients$beta)
eZg <- sigma * exp(as.numeric(Z %*% M$coefficients$gamma/2))
eps_est <- uXb/eZg
# display the nu estimator
quantile(eps_est,probs = .75)
```
...
The primary purpose of \pkg{stanEL} is to perform Bayesian inference on empirical likelihood by using Stan. For this purpose we must specify a prior on $\beta$ and $\tau$, which we take from @chaudhuri-elhmc :
...
# number of regions
n_area <- nlevels(js_data$region)
# number of observations per region
n_obs <- nrow(tab_yes)

# n_obs x n_area matrix of counts of satisfied employees
y <- tab_yes %>%
  select(-race, -age, -gender) %>%
  as.matrix() 

# n_obs x n_area matrix of total number of employees
N <- tab_total %>%
  select(-race, -age, -gender) %>%
  as.matrix()
...
\subsection[MAP Using flexEL]{MAP Using \pkg{flexEL}}

Let us start by finding the maximum *a posteriori* (MAP) estimates of $\bbe$ and  $\zz$ using the \pkg{flexEL} package.  Note that, instead of writing the log-posterior in terms of $\bbe$ and $\zz$, we instead parametrize it in terms of $\bbe_{\text{raw}} = \bbe/\sigma$ and $\zz_{\text{raw}} = \zz/\tau$, as this typically improves convergence of Stan's MCMC sampler.
...
#' Calculate the `G` matrices for the LOGISA model.
#'
#' @param beta Vector of `n_cov` regression parameters.
#' @param z Vector of `n_area` regional parameters.
#' @param y Matrix of `n_obs x n_area` successes.
#' @param N Matrix of `n_obs x n_area` total counts.
...
#' EL log-posterior for LOGISA model.
#'
#' @param beta_raw Vector of `n_cov` standardized regression parameters `beta_raw = beta/beta_sd`.
#' @param z_raw Vector of `n_area` standardized regional parameters `z_raw = z/sqrt(tau)`.
#' @param tau Variance of regional parameters.
#' @param y Matrix of `n_obs x n_area` successes.
#' @param N Matrix of `n_obs x n_area` total counts.
#' @param X Common covariate matrix of size `n_obs x n_cov`.
#' @param beta_sd Standard deviation of beta.
#' @param tau_prior Length-two vector of shape and scale parameters of the inverse-gamma prior distribution on `tau`.
#' @param supp_adj,max_iter,rel_tol Tuning parameters for [flexEL::logEL()].
#' @return The EL logposterior for the LOGISA model.
logisa_el_logpost <- function(beta_raw, z_raw, tau,
                              y, N, X,
                              beta_sd, tau_prior,
  ...
  ll <- sapply(G_all, flexEL::logEL,
               supp_adj = supp_adj, max_iter = max_iter, rel_tol = rel_tol)
...
nlogpost <- function(theta) {
  ...
  -logisa_el_logpost(beta_raw = beta_raw, z_raw = z_raw, tau = tau,
                     y = y, N = N, X = X,
...
theta_init <- c(rnorm(n_cov + n_area, sd = 0.1), runif(1))

optimout <- optim(par = theta_init, fn = nlogpost,
...
Xb_est <- X %*% beta_est
map_est <- t(ilogit(Xb_est  + z_est[n_area]))
map_est
```


\subsubsection[MCMC using stanEL]{MCMC using \pkg{stanEL}}
...
```{r showstanEL, echo = FALSE, eval = TRUE, results = "asis"}
cat("```cpp",
...
<!-- ```{stan, output.var = "logisastan", eval = FALSE, code = readLines(system.file("stan", "logisa_el.stan", package = "stanEL"))} -->
<!-- ``` -->

We now turn to MCMC sampling with \pkg{rstan}:

```{r stanEL, fig.width=7, fig.height= 4, message=FALSE, warning=FALSE}
beta_sd <- 10
tau_prior <- c(.1,.1)
...
# uncomment the line below to do sampling in parallel
## options(mc.cores = parallel::detectCores())
...
- `MAP`: The Bayesian MAP estimate using \pkg{flexEL}.

- `NUTS`: The posterior mean of the full Bayesian estimate using \pkg{stanEL}.
...
# nuts estimate using stanEL
nuts_est <- summary(job_satis_fit,pars = 'rho_Pacific')[[1]][,1]
# hmc estimate reported by Chaudhuri et al 2017
cmy_est <- c(0.636,0.628,.677,.669,.733,.726,.674,.583,.713,.626,.764,.686)

disp <- rbind(glm_est, map_est, nuts_est, cmy_est)
rownames(disp) <- c("GLM", "MAP", "NUTS", "CMY17")
round(disp, 3)
```
