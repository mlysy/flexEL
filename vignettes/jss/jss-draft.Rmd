...
Let $\e_1,\cdots,\e_n$ where $\e_i\in\R$ be iid observations from an unknown distribution $F_0(\e)$, about which a parameter of interest $\tth$ is defined as satisfying an $m$-dimensional moment condition:
\begin{equation} \label{eq:momcond}
  \E\bigl[\gg(\eep;\tth)\bigr] = 0,
\end{equation}
where $\gg(\eep, \tth) = \bigl( g_1(\eep, \tth), \ldots, g_m(\eep, \tth) \bigr)$.

The empirical likelihood $\EL(\tth)$ as the profile likelihood over the distribution function of $\e$:
...
\begin{equation} \label{eq:lagrange}
  \L = \sum_{i=1}^n q_i\log(\w_i) + (\sum_{i=1}^n q_i)\lla'(\sum_{i=1}^n \w_i \cdot g_i(\eep;\tth)) + \mu(1-\sum_{i=1}^n \w_i).
\end{equation}
