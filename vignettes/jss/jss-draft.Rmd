...
Users can also leverage the EL calculation for customized models specified by estimating equations, in which case, a user can either create a function in \proglang{R} that returns the `G` matrix given the value of the parameters (called e.g. `evalG`), or provide the calculation in C++ before wrapping into an \proglang{R} interface. 
...
library(flexEL)
mr_neg_logel <- function(beta, y, X, gel) {
...
This can be implemented in R (called `mr_evalG`) as follows: 
```{r, echo=TRUE, eval=TRUE}
...
```{r, echo=TRUE, eval=TRUE}
mr_neglogel_R <- function(beta, y, X, gel) {
  G <- mr_evalG_R(y, X, beta)
  return(-gel$logel(G))
}
```

One may also provide the gradient of `evalG` with respect to the parameters, in this case, it is
```{r, echo=TRUE, eval=TRUE}
...
```{r, echo=TRUE, eval=TRUE}
mr_neglogel_grad_R <- function(beta, y, X, gel) {
  G <- mr_evalG_R(y, X, beta)
...
optim(c(0.5,2.5), fn = mr_neglogel_R, gr = mr_neglogel_grad_R,
      y, X, gel, method = "BFGS")
```
