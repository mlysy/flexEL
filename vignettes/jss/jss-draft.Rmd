...
Finally, we perform the optimization to obtain the parameter estimate of $\bbe$.
...
Next, we follow an example introduced in @huang18. In the data simulation, we set $\xx_i = \left[1,x_i\right]'$, $\bbe = \left[\beta_0, \beta_1\right]' = \left[0.5,1\right]'$, $\sigma^2 = 1$. Also, we let $x_i,z_i \iid \N(0,1)$,
the censoring variable $c_i \iid \N(1.35,1)$, and $\e_i$
