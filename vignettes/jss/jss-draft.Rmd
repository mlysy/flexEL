...
We first show the simple functional implementation of \pkg{flexEL}. By creating the function `mr_evalG_R()` returning `G` matrix and `mr_neglogel()` returning negative logEL value, one can use `nlm()` to minimize the negative logEL value and get $\bbe$ estimate. 
...
#' return negative logEL value with the gel class
...
- [ ] Write down the derivative of the smoothed check function $\frac{\ud}{\ud u} \rho_{S,\tau}(u; s)$, and complete the coding in `check_smooth_dx()`.
...
QuantReg <- R6Class(
  ...
  public = list(
    ...
    evalG = function(beta, gamma, sigma, nu, tau, s) {
    ...
    logel = function(beta, gamma, sigma, nu, tau, s) {
      eps <- self$resid(beta, gamma, sigma)
      G <- evalG(beta, gamma, sigma, nu, tau, s)
