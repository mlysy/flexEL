...
\begin{equation}
\pi(\tth\mid\xx) = \frac{L(\tth\mid\xx)\pi(\tth)}{\int L(\tth\mid\xx)\pi(\tth) \ud \tth}
...
Let us consider a simple linear regression as an example. Suppose we have $n$ independent and identically distributed observations $(x_i,y_i)$, $i = 1,...,n$, which follow the model 
\begin{equation}
y_i = \beta_0 + \beta_1x_i + \epsilon_i,
\end{equation}
where $\epsilon_i$ follows an unknown distribution with $\E(\epsilon_i) = 0$.

To implememnt the empirical likelihood, the moment condition in this example is 
\begin{equation}
\gg(\xx, y; \bbe) = (y - \xx'\bbe) \xx.
\end{equation}
