...
Let $\e_1,\cdots,\e_n$ where $\e_i\in\R^{d+1}$ be iid observations from an unknown distribution $F_0(\eep)$, about which a parameter of interest $\tth$ is defined as satisfying an $m$-dimensional moment condition:
\begin{equation} \label{eq:momcond}
  \E\bigl[\gg(\eep;\tth)\bigr] = 0,
\end{equation}
where $\gg(\eep, \tth) = \bigl( g_1(\eep, \tth), \ldots, g_m(\eep, \tth) \bigr)$.

The empirical likelihood $\EL(\tth)$ as the profile likelihood over the distribution function of $\eep$:
\begin{equation} \label{eq:elF}
  \EL(\tth) = \max_{F \in \mathcal F(\tth)} \prod_{i=1}^n \mathrm{d} F(\e_i),
\end{equation}
where for any given $\tth$, $\mathcal F(\tth)$ is the set of (valid) distribution functions satisfying \eqref{eq:momcond}.

For any $\tth$, @owen1988 has shown that the maximum of \eqref{eq:elF} can be achieved by focusing on the distribution functions having all mass on the support of the observed data $\e_1, \cdots, \e_n$, and the infinite-dimensional profile likelihood \eqref{eq:elF} can be reduced to a finite-dimensional one. 
...
\begin{split}
  ...
  \text{s.t.}\quad & \sum_{i=1}^n \w_i\cdot g_i(\eep;\tth) = 0 \\
  & \sum_{i=1}^n \w_i = 1 \\
  & \w_i \geq 0, \quad i=1,\cdots,n.
\end{split}
\end{equation}

When $q_i = 1, \forall i=1,\cdots,n$, we have the regular empirical likelihood function. The problem in (\ref{eq:noncensopt}) is a constrained convex optimization problem, and its optimal solution can be found via Lagrangian function, similar to the steps in @owen1990. Specifically, the Lagrangian funciton can be set up as 
\begin{equation} \label{eq:lagrange}
  \L = \sum_{i=1}^n q_i\log(\w_i) + n\lla'(\sum_{i=1}^n \w_i \cdot g_i(\eep;\tth)) - \mu(\sum_{i=1}^n \w_i -1).
\end{equation}

Let $r_i = q_i/\sum_{i=1}^n q_i$. Provided that $\bm 0$ is in the convex full of the points $g_1(\eep;\tth),\cdots,g_n(\eep;\tth)$, a unique optimal probability vector exist and can be shown to be
\begin{equation}
  \hat{\w_i}(\tth) = \frac{r_i}{1 - \hat{\lla}'(\tth) g_i(\eep;\tth)},
\end{equation}
where 

\begin{equation}
\begin{aligned}
\hat{\lla}(\tth) &= \argmax_{\lla(\tth)} \sum_{i=1}^n r_i\ \str\log\left(1 - \lla'(\tth) g_i(\eep;\tth); r_i\right), \\
\str\log(x; r) &= 
\begin{cases} 
\log(x) & x \ge r \\
...
@qin-lawless1994 has shown that $\lla(\tth)$ is a continuous differentiable function of $\tth$ provided that convex hull condition is satisfied with $\tth$ and $\sum_{i=1}^n g_i(\eep;\tth)g_i'(\eep;\tth)$ is positive definite. However, the support of $\tth$ is not necessarily a convex set, as demonstrated by @chaudhuri-et-al2017.
...
Let's denote the residuals given a specific $\tth$ as $e_i'$s (corresponding to $u_i'$s), the complete residuals as $\e_i'$ (corresponding to $y_i'$s). Recall that if $\delta_i=1$, we have $e_i=\e_i$. This means that the \textbf{unobserved (latent) variables} are the $\e_i'$s such that $\delta_i=0$, the \textbf{complete data likelihood} is
...
\begin{split}
  \max_{\w}\quad & \sum_{i=1}^n q_i \log \w_i \\
  \mbox{s.t.}\quad & \sum_{i=1}^n \w_i\cdot \gg(x_i,u_i;\tth) = 0 \\
  & \sum_{i=1}^n \w_i = 1 \\
  & \w_i \geq 0, \quad i=1\cdots,n,
\end{split}
\end{equation}

<!-- which is in the same form as in the case without right-censoring. It can be shown that (see Appendix \ref{app:mstep}) the solution of (\ref{eq:emmstep}) is -->

\begin{equation}
  \hat\w_i = \frac{q_i}{n + \hat{\lla}'(\tth) \gg(x_i,u_i;\tth)},
\end{equation}

where $\hat{\lla}(\tth)$ can be solved analogously which is

\begin{equation}\label{eq:lhatcens}
    \hat\l(\tth) = \argmax_{\lla} \sum_{i=1}^n q_i\cdot\sha\log\Bigl(n + \lla' \gg(x_i,u_i;\tth)\Bigr),
\end{equation}

where
\begin{equation}\label{eq:logsharp}
  \sha\log(x_i; q_i) = \begin{cases} \log(x_i) & x_i \ge q_i \\
   -\frac{1}{2q_i^2} x_i^2 + \tfrac 2 q_i x_i - \tfrac 3 2 + \log(q_i) & x_i < q_i \end{cases}.
\end{equation}
