...
test_that("logel.R == logel.cpp", {
    for(ii in 1:ntest) {
        ...
        if (sum(omegas) != 0) {
            ...
            expect_gt(logopt.cpp - logtwk.cpp, 0)
        }
...
test_that("logelC.R == logelC.cpp", {
    for(ii in 1:ntest) {
        ...
        G <- matrix(rnorm(n*p),n,p)
        deltas <- rep(1,n)
        censnum <- sample(n-8,1) # number of points censored 
        deltas[sample(n,censnum)] <- 0
        epsilons <- rnorm(n)
        omegas.cpp <- omega.hat(G, deltas, epsilons) # optimal omegas (by C++ here)
        ...
        if (sum(omegas) != 0) {
            ...
            # check C++ and R are the same 
            logopt.cpp <- logEL(omegas, G, deltas, epsilons)
            logopt.R <- logEL_R(omegas, G, deltas, epsilons)
            expect_equal(logopt.cpp,logopt.R)
            ...
            # this should be 0s still 
            expect_equal(c(omegas_twk %*% G), rep(0,p))
            logtwk.cpp <- logEL(omegas_twk, G, deltas, epsilons)
            # logopt should be greater than logtwk 
            expect_gt(logopt.cpp - logtwk.cpp, 0)
        }
