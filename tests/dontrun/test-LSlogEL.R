...
plot(y)
beta.seq <- seq(-.2+beta0, .2+beta0, length.out = numpoints)
gamma.seq <- seq(-.2+gamma0, .2+gamma0, length.out = numpoints)
theta.seq <- rbind(beta.seq, gamma.seq)
logel.seq <- matrix(rep(NA,2*numpoints),2,numpoints)
for (ii in 1:numpoints) {
    logel.seq[1,ii] <- mrls.logel(y, X, X, c(theta.seq[1,ii]), c(gamma0))
    logel.seq[2,ii] <- mrls.logel(y, X, X, beta0, theta.seq[2,ii])
}
