...
# smoothed evalWeight function with beta prior
evalWeights.smooth_R <- function(deltas, omegas, epsilons, a=0.8) {
  nObs <- length(omegas)
  epsOrd <- order(epsilons)
  psots <- rep(0,nObs)
  for (ii in 1:nObs) {
    for (jj in 1:nObs) {
      psots[ii] <- psots[ii] + (1-deltas[jj])*((epsilons[jj]>=epsilons[ii]+s)/(nObs+2*a))*
        omegas[ii]/evalPsos_R(jj,epsOrd,omegas)
    }
  }
