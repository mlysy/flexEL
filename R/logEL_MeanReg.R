...
#' @details ...
#' @export
logEL_MeanReg <- function(y, X, nObs, nEqs, beta, lambda0, max_iter = 100, eps = 1e-7) {
      # input checks
      if(ncol(X) != length(y)) {
      ...
      .logEL_MeanReg(y, X, nObs, nEqs, beta, lambda0, maxIter = 100L, eps = 1e-7)
      # .logELmean(nObs, nEqs, y, X, beta, lambda0, maxIter, eps)
}
