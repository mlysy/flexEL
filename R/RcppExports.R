# Generated by using Rcpp::compileAttributes() -> do not edit by hand
# Generator token: 10BE3573-1514-4C36-9D1C-5A225CD40393

.getWeights <- function(y, X, deltas, omegas, epsilons) {
    .Call(`_bayesEL_getWeights`, y, X, deltas, omegas, epsilons)
}

.lambdaNRC <- function(G, weights, maxIter, relTol, verbose) {
    .Call(`_bayesEL_lambdaNRC`, G, weights, maxIter, relTol, verbose)
}

.omega.hat.EM <- function(G, deltas, epsilons, maxIter, relTol, verbose) {
    .Call(`_bayesEL_getOmegasEM`, G, deltas, epsilons, maxIter, relTol, verbose)
}

.lambdaNR <- function(G, maxIter, relTol, verbose) {
    .Call(`_bayesEL_lambdaNR`, G, maxIter, relTol, verbose)
}

.omega.hat <- function(G, maxIter, relTol, verbose) {
    .Call(`_bayesEL_getOmegas`, G, maxIter, relTol, verbose)
}

.MeanReg_evalG <- function(y, X, beta) {
    .Call(`_bayesEL_MeanReg_evalG`, y, X, beta)
}

.MeanReg_logEL <- function(y, X, beta, maxIter = 100L, relTol = 1e-7) {
    .Call(`_bayesEL_MeanReg_logEL`, y, X, beta, maxIter, relTol)
}

.QuantReg_logEL <- function(y, X, alpha, beta, maxIter = 100L, relTol = 1e-7) {
    .Call(`_bayesEL_QuantReg_logEL`, y, X, alpha, beta, maxIter, relTol)
}

