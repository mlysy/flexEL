...
#' @param ws Optional length-\code{nObs} vector of weights for weighted Newton-Raphson algorithm.
...
#' @export
lambdaNR <- function(G, ws, max_iter = 100, rel_tol = 1e-7, verbose = FALSE) { 
    # check whether ws is given and call the corresponding NR funciton
    if (missing(ws)) {
        ans <- .lambdaNR(G = t(G), maxIter = max_iter, rel_tol = rel_tol, verbose = verbose)
    }
    else {
        ans <- .lambdaNRC(G = t(G), ws, maxIter = max_iter, rel_tol = rel_tol, verbose = verbose)
    }
