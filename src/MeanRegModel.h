 ...
 */
inline MeanRegModel::MeanRegModel(int n_obs, int n_eqs) {
  n_obs_ = n_obs;
 ...
 */
inline void MeanRegModel::set_data(const Ref<const VectorXd>& y,
                                  const Ref<const MatrixXd>& X) {
  y_ = y;
  X_ = X;
  n_obs_ = y.size();
...
inline int MeanRegModel::get_n_obs() {
  return n_obs_;
}

inline int MeanRegModel::get_n_eqs() {
  return n_eqs_;
}
