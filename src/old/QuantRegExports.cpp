...
// [[Rcpp::export(".logEL_QuantReg")]]
double logEL_QuantReg(Eigen::VectorXd y, Eigen::MatrixXd X, int nObs, int nEqs, 
                 Eigen::VectorXd beta, double alpha,
                 Eigen::VectorXd lambda0, int maxIter = 100, double eps = 1e-7) {
      InnerEL<QuantRegModel> QR(y, X, alpha, nObs, nEqs, lambda0); // instantiate
