...
public:
  InnerELC(int, int, VectorXd); // constructor
  // for accessibility
  int getnObs() const;
  int getnDims() const;
...
// constructor
inline InnerELC::InnerELC(int N, int m, VectorXd lambda0) {
  nObs = N;
  nDims = m;
  // logstar constants
  trunc = 1.0/nObs;
...
// for accessiblity
inline int InnerELC::getnObs() const {return nObs;}
inline int InnerELC::getnDims() const {return nDims;} 

// logstar
inline double InnerELC::logstar(double x) {
  if(x >= trunc) {
    return(log(x));
  } else {
    return((aa*x + bb)*x + cc);
...
// d logstar(x)/dx
inline double InnerELC::logstar1(double x) {
  if(x >= trunc) {
    return(1.0/x);
  } else {
    // return(aa*x + bb);
...
// d^2 logstar(x)/dx^2
inline double InnerELC::logstar2(double x) {
  if(x >= trunc) {
    return(-1.0/(x*x));
  } else {
    // return(aa);
...
// GGt = [g1 g1' ... gN gN']
inline void InnerELC::BlockOuter(void) {
...
// maximum relative error in lambda
inline double InnerELC::MaxRelErr(const Ref<const VectorXd>& lambdaNew,
                                 const Ref<const VectorXd>& lambdaOld) {
...
// Newton-Raphson algorithm
inline void InnerELC::LambdaNR(int& nIter, double& maxErr,
                              int maxIter, double tolEps) {
  int ii, jj;
...
inline MatrixXd InnerELC::PostSample(int nsamples, int nburn,
  ...
  for (int ii=-nburn; ii<nsamples; ii++) {
    for (int jj=0; jj<betalen; jj++) {
      ...
      InnerELC::LambdaNR(nIter, maxErr, maxIter, eps);
